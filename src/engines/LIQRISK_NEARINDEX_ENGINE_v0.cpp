...
SCSFExport scsf_LIQRISK_NEARINDEX_ENGINE_v0(SCStudyInterfaceRef sc)
{
  if(sc.SetDefaults){
    sc.GraphName="LIQRISK_NEARINDEX_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=0; sc.GraphRegion=0; sc.ValueFormat=26; sc.FreeDLL=0;
  ...
  double pos = (atr>0? (sc.Close[idx]-sc.Low[idx])/atr : 0.0); if(pos<0) pos=0; // 0..~range/atr
  sc.Subgraph[1][idx]=pos; sc.Subgraph[2][idx]=atr;
}
