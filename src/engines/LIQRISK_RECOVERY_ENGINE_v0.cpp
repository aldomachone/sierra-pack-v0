...
SCSFExport scsf_LIQRISK_RECOVERY_ENGINE_v0(SCStudyInterfaceRef sc)
{
  if(sc.SetDefaults){
    sc.GraphName="LIQRISK_RECOVERY_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=0; sc.GraphRegion=0; sc.ValueFormat=26; sc.FreeDLL=0;
  ...
  // Recovery = distance du prix au plus bas des N barres en unités d'ATR
  double lo=1e300; for(int k=s;k<=idx;++k) if(sc.Low[k]<lo) lo=sc.Low[k];
  double rec = (atr>0? (sc.Close[idx]-lo)/atr : 0.0);
  sc.Subgraph[1][idx]=rec;
}
