...
SCSFExport scsf_EFFICIENCY_RATIO_ENGINE_v0(SCStudyInterfaceRef sc)
{
  ...
  if(sc.ArraySize<2) return; int idx=sc.ArraySize-1; int N=sc.Input[0].GetInt(); int s=(idx-N>0? idx-N:0);
  double change=fabs(sc.Close[idx]-sc.Close[s]); double vol=0; for(int i=s+1;i<=idx;++i) vol+=fabs(sc.Close[i]-sc.Close[i-1]);
  sc.Subgraph[1][idx]=(vol>0? change/vol:0.0);
}
