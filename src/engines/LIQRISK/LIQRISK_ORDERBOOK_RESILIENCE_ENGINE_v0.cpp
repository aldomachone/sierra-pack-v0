...
SCSFExport scsf_LIQRISK_ORDERBOOK_RESILIENCE_ENGINE_v0(SCStudyInterfaceRef sc)
{
  int& state=sc.GetPersistentInt(1); double& base=sc.GetPersistentDouble(2); int& t0=sc.GetPersistentInt(3);
  if(sc.SetDefaults){
    sc.GraphName="LIQRISK_ORDERBOOK_RESILIENCE_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=1; sc.GraphRegion=0; sc.ValueFormat=26; sc.UsesMarketDepthData=1; sc.FreeDLL=0;
    ...
    sc.Input[2].Name="03. Recovery %"; sc.Input[2].SetFloat(0.7f);
  ...
  if(sc.IsFullRecalculation || base<=0) base=sum;
  double frac=(sum>0? sum/base:0.0);
  if(state==0 && frac<=1.0-sc.Input[1].GetFloat()){ state=1; t0=idx; }
  if(state==1 && frac>=sc.Input[2].GetFloat()){ sc.Subgraph[1][idx]=idx-t0; state=0; base=sum; }
}
