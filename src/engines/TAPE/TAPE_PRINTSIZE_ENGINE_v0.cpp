...
SCSFExport scsf_TAPE_PRINTSIZE_ENGINE_v0(SCStudyInterfaceRef sc)
{
  int& inited=sc.GetPersistentInt(1);
  double& emaSz=sc.GetPersistentDouble(2);

  if(sc.SetDefaults)
  {
    sc.GraphName="TAPE_PRINTSIZE_ENGINE_v0";
  ...
  if(!inited || sc.IsFullRecalculation){ inited=1; emaSz=0; }
  ...
  { const auto& e=ts[i]; if(e.Type!=SC_TS_TRADES) continue; emaSz = a*e.Volume + (1-a)*emaSz; }
  int idx=sc.ArraySize-1; if(idx>=0) sc.Subgraph[1][idx]=emaSz;
}
