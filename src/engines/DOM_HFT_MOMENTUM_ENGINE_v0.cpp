...
SCSFExport scsf_DOM_HFT_MOMENTUM_ENGINE_v0(SCStudyInterfaceRef sc)
{
  int& inited=sc.GetPersistentInt(1); double& ema=sc.GetPersistentDouble(2);
  if(sc.SetDefaults){
    sc.GraphName="DOM_HFT_MOMENTUM_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=1; sc.GraphRegion=0; sc.ValueFormat=26; sc.UsesMarketDepthData=1; sc.FreeDLL=0;
  ...
  s_MarketDepthEntry md{}; static double prev=0; double q=0;
