...
SCSFExport scsf_REALIZED_QUANTILE_VAR_ENGINE_v0(SCStudyInterfaceRef sc)
{
  if(sc.SetDefaults){
    sc.GraphName="REALIZED_QUANTILE_VAR_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=0; sc.GraphRegion=0; sc.ValueFormat=26;
  ...
  double q=sc.Input[1].GetFloat(); if(q<0) q=0; if(q>1) q=1;
  int k=(int)(q*(n-1)); if(k<0) k=0; if(k>=n) k=n-1;
  sc.Subgraph[1][idx]=r[k];
}
