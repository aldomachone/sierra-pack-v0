...
SCSFExport scsf_PROFILE_BALANCE_ENGINE_v0(SCStudyInterfaceRef sc)
{
  if(sc.SetDefaults){
    sc.GraphName="PROFILE_BALANCE_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=0; sc.GraphRegion=0; sc.ValueFormat=26; sc.FreeDLL=0;
  ...
  double bal = 1.0 - fabs((poc - 0.5*(l+r)))/(0.5*(r-l+1)); // proxy 0..1
  int idx=sc.ArraySize-1; sc.Subgraph[1][idx]=bal;
}
