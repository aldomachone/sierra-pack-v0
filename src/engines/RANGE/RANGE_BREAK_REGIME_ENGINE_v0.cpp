...
SCSFExport scsf_RANGE_BREAK_REGIME_ENGINE_v0(SCStudyInterfaceRef sc)
{
  if(sc.SetDefaults){
    sc.GraphName="RANGE_BREAK_REGIME_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=0; sc.GraphRegion=0; sc.ValueFormat=26; sc.FreeDLL=0;
  ...
  double bw=hi-lo; double regime = (atr>0? bw/atr:0.0); // petit=trend, grand=range
  sc.Subgraph[1][idx]=regime; sc.Subgraph[2][idx]=bw; sc.Subgraph[3][idx]=atr;
}
