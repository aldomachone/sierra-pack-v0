...
SCSFExport scsf_RANGE_MEANREVERT_SCORE_ENGINE_v0(SCStudyInterfaceRef sc)
{
  if(sc.SetDefaults){
    sc.GraphName="RANGE_MEANREVERT_SCORE_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=0; sc.GraphRegion=0; sc.ValueFormat=26; sc.FreeDLL=0;
  ...
  double dist = fmin(fabs(sc.Close[idx]-hi), fabs(sc.Close[idx]-lo))/atr;
  sc.Subgraph[1][idx]=1.0/(1.0+dist);
}
