...
SCSFExport scsf_OFI_BOOK_MOMENTUM_ENGINE_v0(SCStudyInterfaceRef sc)
{
  int& inited=sc.GetPersistentInt(1); double& pb=sc.GetPersistentDouble(2); double& pa=sc.GetPersistentDouble(3);
  if(sc.SetDefaults){
    sc.GraphName="OFI_BOOK_MOMENTUM_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=1; sc.GraphRegion=0; sc.ValueFormat=26; sc.UsesMarketDepthData=1; sc.FreeDLL=0;
  ...
  sc.Subgraph[1][sc.ArraySize-1]=ofi;
}
