...
SCSFExport scsf_VWAP_BIAS_STATE_ENGINE_v0(SCStudyInterfaceRef sc)
{
  if(sc.SetDefaults){
    sc.GraphName="VWAP_BIAS_STATE_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=0; sc.GraphRegion=0; sc.ValueFormat=26; sc.FreeDLL=0;
  ...
  double sd=(var>0? sqrt(var):sc.TickSize); double k=sc.Input[1].GetFloat();
  double dist=(sd>0? (sc.Close[idx]-vwap)/sd:0.0);
  double state = (dist>k? +2.0 : (dist>0? +1.0 : (dist<-k? -2.0 : (dist<0? -1.0 : 0.0))));
  sc.Subgraph[1][idx]=state; sc.Subgraph[2][idx]=dist; sc.Subgraph[3][idx]=vwap;
}
