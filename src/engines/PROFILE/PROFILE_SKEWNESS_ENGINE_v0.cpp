...
SCSFExport scsf_PROFILE_SKEWNESS_ENGINE_v0(SCStudyInterfaceRef sc)
{
  if(sc.SetDefaults){
    sc.GraphName="PROFILE_SKEWNESS_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=0; sc.GraphRegion=0; sc.ValueFormat=26; sc.FreeDLL=0;
  ...
  double s2=0, s3=0; for(int i=0;i<B;++i){ double d=i-m; s2+=d*d*bins[i]; s3+=d*d*d*bins[i]; }
  if(s2<=0) return; double skew = (s3/tot)/pow(s2/tot,1.5);
