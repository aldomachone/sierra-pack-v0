...
{
  int& inited=sc.GetPersistentInt(1); double& emaPoc=sc.GetPersistentDouble(2);
  if(sc.SetDefaults){
  ...
  if(!inited||sc.IsFullRecalculation){ inited=1; emaPoc=0; } if(sc.ArraySize<=0) return;
  int lb=sc.Input[0].GetInt(); int start=sc.ArraySize-lb; if(start<0) start=0;
  // POC proxy: prix de volume max
  ...
  double a=sc.Input[1].GetInt()/100.0; emaPoc=a*poc+(1-a)*emaPoc;
  int idx=sc.ArraySize-1; sc.Subgraph[1][idx]=emaPoc; sc.Subgraph[2][idx]=poc-emaPoc; // shift
}
