...
SCSFExport scsf_DOM_QUEUE_SHAPE_ENGINE_v0(SCStudyInterfaceRef sc)
{
  if(sc.SetDefaults){
    sc.GraphName="DOM_QUEUE_SHAPE_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=1; sc.GraphRegion=0; sc.ValueFormat=26; sc.UsesMarketDepthData=1; sc.FreeDLL=0;
  ...
  double near=bN+aN, mid=bM+aM, far=bF+aF;
  int idx=sc.ArraySize-1; if(idx>=0){ sc.Subgraph[1][idx]=near; sc.Subgraph[2][idx]=mid; sc.Subgraph[3][idx]=far; sc.Subgraph[4][idx]=(near>0? mid/near:0.0); sc.Subgraph[5][idx]=(near>0? far/near:0.0); }
}
