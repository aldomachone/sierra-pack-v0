...
SCSFExport scsf_DOM_IMPACT_MICROPRICE_ENGINE_v0(SCStudyInterfaceRef sc)
{
  if(sc.SetDefaults){
    sc.GraphName="DOM_IMPACT_MICROPRICE_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=1; sc.GraphRegion=0; sc.ValueFormat=26; sc.FreeDLL=0; sc.UsesMarketDepthData=1;
  ...
  double mp = (b+aQ>0? (aQ*sc.Bid[sc.ArraySize-1] + b*sc.Ask[sc.ArraySize-1])/(aQ+b) : sc.Close[sc.ArraySize-1]);
  e = a*mp + (1-a)*e;
  int idx=sc.ArraySize-1; if(idx>=0){ sc.Subgraph[1][idx]=e; sc.Subgraph[2][idx]=mp-e; }
}
