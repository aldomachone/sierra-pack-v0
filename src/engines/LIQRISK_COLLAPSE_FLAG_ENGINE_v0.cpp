...
SCSFExport scsf_LIQRISK_COLLAPSE_FLAG_ENGINE_v0(SCStudyInterfaceRef sc)
{
  int& inited=sc.GetPersistentInt(1); double& prev=sc.GetPersistentDouble(2);
  if(sc.SetDefaults){
    sc.GraphName="LIQRISK_COLLAPSE_FLAG_ENGINE_v0"; sc.AutoLoop=0; sc.UpdateAlways=1; sc.GraphRegion=0; sc.ValueFormat=26; sc.UsesMarketDepthData=1; sc.FreeDLL=0;
  ...
  int flag = (prev>0 && sum/prev <= (1.0 - sc.Input[1].GetFloat()) ? 1:0); prev=sum;
