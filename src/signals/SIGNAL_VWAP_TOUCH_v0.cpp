#include "sierrachart.h"
  ...
  SCSFExport scsf_SIGNAL_VWAP_TOUCH_v0(SCStudyInterfaceRef sc)
  {
    SCSubgraphRef SG = sc.Subgraph[0];

    if (sc.SetDefaults)
    {
      sc.GraphName = "VWAP Touch v0";
      sc.AutoLoop = 0;
      sc.UpdateAlways = 1;
      sc.GraphRegion = 0;
      sc.ValueFormat = 26;
...
double Result = fabs(sc.Close[last]-vwap) / (fabs(vwap)>0.0? fabs(vwap):1.0);
