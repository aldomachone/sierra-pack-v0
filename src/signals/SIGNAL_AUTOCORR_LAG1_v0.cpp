#include "sierrachart.h"
  ...
  SCSFExport scsf_SIGNAL_AUTOCORR_LAG1_v0(SCStudyInterfaceRef sc)
  {
    SCSubgraphRef SG = sc.Subgraph[0];

    if (sc.SetDefaults)
    {
      sc.GraphName = "Autocorr Lag-1 v0";
      sc.AutoLoop = 0;
      sc.UpdateAlways = 1;
      sc.GraphRegion = 0;
      sc.ValueFormat = 26;
...
for(int i=s+1;i<=last;++i){ double r=sc.Close[i]-sc.Close[i-1]; double r1=sc.Close[i-1]-sc.Close[i-2]; num+=r*r1; d1+=r*r; d2+=r1*r1; }
