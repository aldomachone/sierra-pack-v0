#include "sierrachart.h"
  ...
  SCSFExport scsf_SIGNAL_ANOM_RANGE_Z_v0(SCStudyInterfaceRef sc)
  {
    SCSubgraphRef SG = sc.Subgraph[0];
    if (sc.SetDefaults)
    {
      sc.GraphName = "Anomaly Range Z v0";
      sc.AutoLoop = 0;
      sc.UpdateAlways = 1;
      sc.GraphRegion = 0;
      sc.ValueFormat = 26;
...
SCInputRef In_01_Z = sc.Input[0]; In_01_Z.Name = "01. Z"; In_01_Z.SetFloat(3.0); // Seuil
  ...
  static double m=0.0,s2=0.0; static int k=0; double x=fabs((double)sc.High[last]- (double)sc.Low[last]);
