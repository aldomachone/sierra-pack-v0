...
/*
// Fournir une couche d'abstraction propre pour manipuler :
...
inline void 		CopyArray					(SCFloatArrayRef dest, const SCFloatArrayRef src							)	{
const int n 	=	src.GetArraySize()	// Copie d'une série dans une autre (resize automatique)																																										;
...
int count 		=	0																																															;
while (sc.GetBidMarketDepthEntryAtLevel(e,count))++count																																						;
return count																																																	;
}
...
inline void			ApplyHeadlessDefaults		(SCStudyInterfaceRef sc)														{
  ...
  for (int i = 0; i < SC_MAX_NUM_STUDY_SUBGRAPHS; ++i)// Aucun SG visible / aucune valeur affichée par défaut
  {
