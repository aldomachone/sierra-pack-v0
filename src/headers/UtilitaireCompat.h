...
inline bool 		tryGetStudyValue		(const SCStudyInterfaceRef& sc, int studyId, int subgraphIdx, int barIndex, float& v){
	SCFloatArray arr; if(!getStudyArray(sc, studyId, subgraphIdx, arr)) return false; 
	const int n=arr.GetArraySize(); if(barIndex<0||barIndex>=n) return false; v=arr[barIndex]; return true; }
