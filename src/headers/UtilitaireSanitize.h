...
inline double 		duMeanSafe				(const double* p,int n) 									{
  ...
  for(int i=0;i<n;++i) if(std::isfinite(p[i])){ s+=p[i]; ++c; }
  return c>0 ? s/c : 0.0;
}

inline double 		duStd					(const double* p,int n) 									{
  if(!p||n<=1) return 0.0;
  double m=duMeanSafe(p,n),v=0.0; int c=0;
  for(int i=0;i<n;++i){ double d=p[i]-m; v+=d*d; ++c; }
...
inline double 		duWeightedMean			(const double* x,const double* w,int n)						{
  ...
  for(int i=0;i<n;++i)
    if(std::isfinite(x[i])&&std::isfinite(w[i])){ sx+=x[i]*w[i]; sw+=w[i]; }
...
inline double 		duCov					(const double* a,const double* b,int n)						{
  ...
  for(int i=0;i<n;++i)
    if(std::isfinite(a[i])&&std::isfinite(b[i])){ s+=(a[i]-ma)*(b[i]-mb); ++c; }
...
// Min/Max/Range
inline double 		duMin					(const double* p,int n)										{ double v=DBL_MAX; for(int i=0;i<n;++i) if(std::isfinite(p[i])&&p[i]<v) v=p[i]; return (v==DBL_MAX)?0.0:v; }
inline double 		duMax					(const double* p,int n)										{ double v=-DBL_MAX;for(int i=0;i<n;++i) if(std::isfinite(p[i])&&p[i]>v) v=p[i]; return (v==-DBL_MAX)?0.0:v; }
...
inline double 		duDist2					(const double* a,const double* b,int n)						{
  ...
  double s=0.0; for(int i=0;i<n;++i){ double d=a[i]-b[i]; s+=d*d; }
  return std::sqrt(s);
}
...
inline double 		duDistNorm				(const double* a,const double* b,int n)						{
  ...
  for(int i=0;i<n;++i){
    ...
    if(std::isfinite(ai)&&std::isfinite(bi)){ s+=std::fabs(ai-bi); ++c; }
  }
  return (c>0)?s/c:0.0;
}

inline double 		duDiff					(double now,double prev,double dt)							{ return duSafeDiv(now-prev,dt,0.0); }
...
inline double 		duEmaSaturating			(double prev, double x, double alphaPct, double limit)		{
  ...
  double diff = e - prev;
  if (std::fabs(diff) > limit) e = prev + duClamp(diff, -limit, limit);
  return e;
}
...
// ----------------------------------------------------------------------------
struct 				ZState 																				{ double mean = 0.0; double var = 0.0; bool init = false; };

inline float 		zEmaUpdate				(ZState& st, float x, int pct) 								{
  if(!std::isfinite(x)) x = 0.0f;
  const float a = clamp(pct, 1, 99) / 100.0f;
  if (!st.init) { st.mean = x; st.var = 0.0; st.init = true; return 0.0f; }
...
inline double 		duZEmaUpdate			(ZState& st,double x,int pct) 								{
  ...
  if(!st.init){ st.mean=x; st.var=0.0; st.init=true; return 0.0; }
...
inline double 		duEntropy01				(double p)													{
  return (p<=0||p>=1)?0.0:-(p*std::log2(p)+(1.0-p)*std::log2(1.0-p));
}

inline bool 		duIsNanInf				(double x)													{ return !std::isfinite(x); }
inline double 		duRound					(double x,int digits)										{ double f=std::pow(10.0,digits); return std::round(x*f)/f; }
inline double 		duTrunc					(double x,int digits)										{ double f=std::pow(10.0,digits); return std::trunc(x*f)/f; }
...
// ----------------------------------------------------------------------------
inline void 		duPrintStats			(const double* p, int n, SCString name="") 					{
  if(!p || n<=0) return;
  double sum=0, sum2=0; int c=0;
  for(int i=0;i<n;++i){ if(std::isfinite(p[i])){ sum+=p[i]; sum2+=p[i]*p[i]; ++c; }}
  if(c>1){
    double mean=sum/c; double var=(sum2/c)-(mean*mean);
    sc.AddMessageToLog(name + ": mean=" + SCString(mean) + " std=" + SCString(std::sqrt(std::fabs(var))),0);
  }
}
...
inline float 		duFastInvSqrt			(float x)													{
  float y=x; long i=*(long*)&y; i=0x5f3759df-(i>>1); y=*(float*)&i;
  return y*(1.5f-0.5f*x*y*y);
}

// Box-Cox transformation
inline double 		duBoxCox				(double x,double lambda)									{
  return (lambda==0.0)?std::log1p(x):((std::pow(1.0+x,lambda)-1.0)/lambda);
}
...
struct 				RangeState 																			{ double min=0, max=0; bool init=false; };
inline void 		duRangeUpdate			(RangeState& st,double x,double alpha)						{
  if(!st.init){ 	st.min=x; st.max=x; st.init=true; return; }
  st.min=duEmaFM	A(st.min,std::min(st.min,x),alpha);
  st.max=duEmaFM	A(st.max,std::max(st.max,x),alpha);
}	
...
#define DU_LOG(x) sc.AddMessageToLog(x,0)
#else
#define DU_LOG(x)
#endif
