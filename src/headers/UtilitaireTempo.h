#pragma once
#include "sierrachart.h"
// ============================================================================
// UtilitaireTempo.h — Helpers de temps/horloge pour ACSIL (Sierra Chart)
// - Conversions SCDateTime → millisecondes
// - Clamps anti-jitter
// - Détection nouvelle journée de trading
// - Estimation skew horloge (système vs chart)
// - Temps de session écoulé
// - Debounce sans état global
// - Utilitaires génériques (clamp, diff sûrs)
//   Conventions : Source unique de temps : nowMs()/duNowMs().

namespace du
{
/////////////////////////////////////////////////////////////////////////////////////////////////////////////// Constantes et conversions de base

constexpr double 	kDayToMs 		=		24.0 * 3600.0 * 1000.0							;
constexpr double 	kSecToMs 		=		1000.0											;

inline long long 	toMs					(SCDateTime t									)	noexcept	// Convertit un SCDateTime (jour julien fractionnaire Sierra) en millisecondes.
{
const double 		ms 				=		static_cast<double>(t) * kDayToMs				;
return static_cast	<long long>				(ms + 0.5)										;
}
inline long long 	nowMs					(const SCStudyInterfaceRef& sc					)	noexcept	// ms depuis epoch selon l’horloge système (Sierra fournit CurrentSystemDateTimeMS).
{
return				toMs					(sc.CurrentSystemDateTimeMS)					;
}
inline long long 	duNowMs					(const SCStudyInterfaceRef& sc					)	noexcept	// Alias de compatibilité (anciens codes : V9G, Stop-Run, etc.).
{
return 				nowMs					(sc)											;
}
inline long long 	barTimeMs				(const SCStudyInterfaceRef& sc, int idx			)	noexcept	// ms de la barre idx selon l’horloge du chart.
{
if 										(sc.ArraySize <= 0)
return 0																					;

int 				maxIndex 		=		sc.ArraySize - 1								;
if 			(idx<0)				idx =		0												;
else if 	(idx > maxIndex)	idx =		maxIndex										;

return 				toMs					(sc.BaseDateTimeIn[idx])						;
}
inline long long 	duNowNs					(SCStudyInterfaceRef							)	noexcept	// Nanosecondes système (non dispo nativement dans ACSIL). Placeholder neutre.
{
return 0																					;
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////	Jours / sessions
// Vrai si la barre courante est dans une nouvelle journée (horloge chart).
// Contrat :
//   - lastYmd doit être initialisé par l’appelant (ex. 0 ou un YMD impossible)
//     avant la première utilisation.
//   - À chaque retour true, lastYmd est mis à jour avec le YMD courant.
inline bool 		newTradingDay			(const SCStudyInterfaceRef& sc, int& lastYmd	)	noexcept
{
  if 										(sc.ArraySize <= 0)
  return false																				;

  int 							idx = 		sc.Index										;
  if 		(idx<0)				idx = 		0												;
  else if 	(idx>=sc.ArraySize)	idx = 		sc.ArraySize - 1								;

  const int 					ymd = 		sc.BaseDateTimeIn[idx].GetDateYMD()				;
  if (ymd != lastYmd)
  {
    lastYmd = ymd																			;
    return true																				;
  }
    return false																			;
}
inline bool 		duIsNewTradingDay		(const SCStudyInterfaceRef& sc, int lastDateYMD	)	noexcept	// Variante “à la volée” sur la dernière barre connue.
{
if (sc.ArraySize <= 0)						return false									;

const int 			last 			= 		sc.ArraySize - 1								;
return 				sc.BaseDateTimeIn[last].GetDateYMD() != lastDateYMD;
}
inline int 			firstIndexOfToday		(const SCStudyInterfaceRef& sc					)	noexcept	// Retourne l’index de la première barre de la journée courante. O(N backscan) borné.
{
  if (sc.ArraySize <= 0)
    return 0;

  int i0 = sc.Index;
  if (i0 < 0)
    i0 = 0;
  else if (i0 >= sc.ArraySize)
    i0 = sc.ArraySize - 1;

  const int ymd = sc.BaseDateTimeIn[i0].GetDateYMD();
  int i = i0;
  while (i > 0 && sc.BaseDateTimeIn[i - 1].GetDateYMD() == ymd)
    --i;
  return i;
}

inline long long 	duSessionElapsedMs		(const SCStudyInterfaceRef& sc					)	noexcept	// Durée écoulée de la session courante en ms, mesurée entre la première barre du jour et l’horloge “chart” de la barre courante.
{
  if (sc.ArraySize <= 0)					return 0										;

  const int       	iStart 			=		firstIndexOfToday	(sc)						;
  const long long 	tStart 			=		barTimeMs			(sc, iStart)				;
  const long long 	tNow   			=		barTimeMs			(sc, sc.Index)				;
  return 			tNow >= tStart ? (tNow - tStart) : 0									;
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////	Jitter / clamps
inline int 			jitterClampMs			(int dtMs, int maxJitterMs						)	noexcept	 // Clamp simple d’un delta de latence en ms
{
  if (dtMs < 0)								return 0										;
  if (dtMs > maxJitterMs)					return maxJitterMs								;
  return 			dtMs																	;
}

template 			<typename T>																			 // Template clamp générique
inline 				T duClamp				(T v, T lo, T hi								)	noexcept
{
  return v < lo ? lo : (v > hi ? hi : v);
}
inline long long 	duSafeDiffMs			(long long newerMs, long long olderMs			)	noexcept	 // Différence ms sûre, non-négative
{
  return newerMs > olderMs ? (newerMs - olderMs) : 0LL;
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////	Échelles temporelles chart
// Secondes équivalentes à N barres.
// - Si SecondsPerBar > 0, on l’utilise directement.
// - Sinon on approxime via la différence temporelle des deux dernières barres.
// - Fallback = N * 1 s.
inline double 		secFromBars				(const SCStudyInterfaceRef& sc, int bars		)	noexcept
{
  if (bars <= 0)
    return 0.0;

  if (sc.SecondsPerBar > 0)
    return static_cast<double>(bars) * static_cast<double>(sc.SecondsPerBar);

  if (sc.ArraySize <= 1)
    return static_cast<double>(bars);

  int i = sc.Index;
  if (i <= 0)
    i = 1;
  else if (i >= sc.ArraySize)
    i = sc.ArraySize - 1;

  const long long t1 = barTimeMs(sc, i);
  const long long t0 = barTimeMs(sc, i - 1);
  const long long dt = duSafeDiffMs(t1, t0);
  if (dt > 0)
    return static_cast<double>(bars) * (static_cast<double>(dt) / kSecToMs);

  return static_cast<double>(bars); // fallback 1 s/bar
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////	Skew horloge
// Estime l’écart horloge système vs horloge chart en secondes.
// Signé : positif si système “en avance” vs chart.
inline double 		duClockSkewEstimate		(const SCStudyInterfaceRef& sc,
                                  long long                 chartNowMs) noexcept
{
  const long long sysMs  = nowMs(sc);
  const long long skewMs = sysMs - chartNowMs;
  return static_cast<double>(skewMs) / kSecToMs;
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////	Debounce sans état global paramétrique
// - lastFireMs 		 : timestamp ms du dernier “fire” (à conserver par l’appelant)
// - nowMsVal   		 : horloge ms courante
// - minGapMs   		 : écart minimal entre deux triggers
// - state      		 : compteur d’événements déclenchés
// Retourne true si un déclenchement est autorisé et met à jour lastFireMs + state

inline bool 		duDebounceMs			(long long& lastFireMs							,
long long  			nowMsVal																,
long long  			minGapMs																,
int&       			state																	)	noexcept
{
  if (minGapMs <= 0)
  {
    lastFireMs = nowMsVal	;
    ++state					;
    return true				;
  }

  const long long dt = duSafeDiffMs(nowMsVal, lastFireMs);
  if (dt < minGapMs)	return false;

  lastFireMs = nowMsVal	;
  ++state				;
  return true			;
}
// Signature initiale “state only” : Stocke le dernier fire dans un PersistentInt64 indexé par l’appelant
inline bool 		duDebounceMs_Persist	(SCStudyInterfaceRef sc							,
int                	persistIdxForLastMs														,
int&               	state																	,
long long          	nowMsVal																,
long long          	minGapMs																)	noexcept
{
  long long& lastMs = sc.GetPersistentInt64(persistIdxForLastMs);
  return duDebounceMs(lastMs, nowMsVal, minGapMs, state);
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////	Helpers divers
inline bool 		duIsNewBar				(const SCStudyInterfaceRef& sc, int& lastIndex	)	noexcept	 // Vrai une fois par nouvelle barre. lastIndex est fourni par l’appelant.
{
  if (sc.Index != lastIndex)
  {
  lastIndex = sc.Index																		;
  return true																				;
  }
  return false																				;
}
inline bool 		duIsFullRecalculation	(const SCStudyInterfaceRef& sc					)	noexcept	 // Proxy pratique sur le flag plateforme.
{
  return 			sc.IsFullRecalculation													;
}
template 			<typename T>
inline void 		duSaturatingAdd			(T& v, T delta, T lo, T hi						)	noexcept	 // Incrémente avec saturation [lo, hi].
{
  if (delta > 0 && v > hi - delta) { v = hi; return; }
  if (delta < 0 && v < lo - delta) { v = lo; return; }
  v += delta;
}

} // namespace du